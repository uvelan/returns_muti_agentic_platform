...
features: list[dict[str, Any]] = [
    ...
    feature("AI-02", "AI Gateway", "Complexity task registry", "Task registry fixes LIGHTWEIGHT/STANDARD tiers; simulator cannot escalate.", VERIFIED, "backend/config/ai_gateway/ (index.yaml + tasks/); backend/src/return_platform/ai_gateway/service.py", "tasks; AIGatewayService.evaluate", test="scripts/validate_stage4n_ai_gateway.py", screen="Missing /ai-gateway/tasks"),
    ...
    feature("AI-04", "AI Gateway", "Bounded retries and deadline", "Bound attempts/backoff and enforce per-route timeout/global deadline.", VERIFIED, "backend/src/return_platform/ai_gateway/service.py; backend/config/ai_gateway/ (index.yaml + tasks/)", "AIGatewayService.evaluate", test="backend/tests/test_ai_gateway_routing.py"),
...
agents = [
    {"Agent": f["Feature"], "Classification": f["Classification"], "Source": f["Source evidence"], "Typed input/output": True, "Boundary": f["Required behavior"], "Prohibited actions": "No authoritative facts or arbitrary SQL", "Configuration": "backend/config/returns/ (index.yaml + parts)", "Invocation": f["API"], "Persistence": f["Persistence"], "Metrics": "agent_decisions", "Tests": f["Test"], "UI": f["Screen"], "Fallback": "Deterministic/human review", "Authority": "Advisory"}
