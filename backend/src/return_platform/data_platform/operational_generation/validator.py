...
def validate_proposal(
    ...
    elif any(f.code in (FindingCode.UNKNOWN_FIELD, FindingCode.INVALID_JSON_SHAPE, FindingCode.DUPLICATE_NATURAL_KEY, FindingCode.MISSING_REQUIRED_FIELD, FindingCode.INVALID_TYPE, FindingCode.INVALID_ENUM, FindingCode.PATTERN_MISMATCH, FindingCode.LENGTH_VIOLATION, FindingCode.RANGE_VIOLATION, FindingCode.INVALID_FOREIGN_KEY, FindingCode.PII_VIOLATION) for f in findings):
...
def validate_field(findings: list[GuardFinding], asset: DataAssetSchema, idx: int, key: str, value: Any, schema: SchemaField) -> None:
    if value is None:
        if schema.required:
            findings.append(GuardFinding(
                code=FindingCode.MISSING_REQUIRED_FIELD,
