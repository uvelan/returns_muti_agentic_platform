...
| `gateway/structured_invocation.py` | `StructuredRouteInvoker` — the structured-output path: any pydantic response model, with the same failover and tier escalation. |
| `gateway/models.py` | API-facing views for routes, tasks, usage, and safety tests. |

### Why there are two entry points, not one

`AIGatewayService.evaluate` and `StructuredRouteInvoker.invoke` are **not** two execution
