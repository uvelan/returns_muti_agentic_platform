    ...
    for index in (1, 2):
        await resolver.put_secret(
            parse_secret_reference(
                f"vault://secret/production/ai/google/credentials/key-{index}#api_key"
            ),
            f"test-key-{index}",
        )
...
async def test_explicit_vault_references_remain_supported() -> None:
    resolver = LocalProxySecretResolver()
    reference = parse_secret_reference("vault://secret/production/ai/google/custom/key#api_key")
