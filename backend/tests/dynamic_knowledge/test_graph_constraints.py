...
def test_required_node_constraints_use_graph_properties_not_field_ids(
    ...
    assert constraints["ConfiguredAlpha"] == ("configured_id",)
    assert constraints["ConfiguredBeta"] == ("related_id",)
...
def test_required_relationship_indexes_cover_both_endpoints(active_schema: ActiveSchema) -> None:
    ...
    assert by_label["ConfiguredAlpha"] == ("configured_id",)
    assert by_label["ConfiguredBeta"] == ("configured_parent_id",)
