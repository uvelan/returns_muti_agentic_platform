...
async def _probe_google_model(client: httpx.AsyncClient, base_url: str, headers: dict[str, str], model: str) -> dict[str, Any]:
    ...
    try:
        ...
        probe_min = await client.post(
            ...
            headers=headers,
            json={
                "contents": [{"role": "user", "parts": [{"text": 'Return only the JSON object {"status":"ok"}.'}]}],
                "generationConfig": {
                    "temperature": 0.0,
                    "maxOutputTokens": 64,
                },
        ...
        if status != 200:
            return {"model": model, "success": False, "error_category": "MINIMAL_PROBE_FAILED", "status": status, "duration_ms": int((t1 - t0) * 1000)}
    except httpx.HTTPError:
        return {"model": model, "success": False, "error_category": "TIMEOUT_OR_NETWORK", "duration_ms": 0}
    ...
    try:
        ...
        probe_struct = await client.post(
            ...
            headers=headers,
            json={
                "contents": [{"role": "user", "parts": [{"text": "Return a JSON object with status ok"}]}],
                "generationConfig": {
                    "temperature": 0.0,
        ...
        if status != 200:
            return {"model": model, "success": False, "error_category": "STRUCTURED_PROBE_FAILED", "status": status, "duration_ms": int((t1 - t0) * 1000)}
        ...
        if "ok" not in text.lower():
            return {"model": model, "success": False, "error_category": "STRUCTURED_OUTPUT_INVALID", "duration_ms": int((t1 - t0) * 1000)}

    except httpx.HTTPError:
        return {"model": model, "success": False, "error_category": "TIMEOUT_OR_NETWORK", "duration_ms": 0}

    # Classification
    tier = "LIGHTWEIGHT" if "flash-lite" in model.lower() or "8b" in model.lower() else "STANDARD"
...
async def _google(settings: Settings) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        tasks = [_probe_google_model(client, settings.google_base_url, selected_headers, model) for model in catalog_models]
...
async def _probe_nvidia_model(client: httpx.AsyncClient, base_url: str, headers: dict[str, str], model: str) -> dict[str, Any]:
    # Probe 2: Minimal text generation
    try:
        t0 = time.monotonic()
        probe_min = await client.post(
            ...
            json={
                "model": model,
                "messages": [{"role": "user", "content": 'Return only the JSON object {"status":"ok"}.'}],
                "temperature": 0.0,
                "max_tokens": 64,
            },
        )
    ...
    tier = "LIGHTWEIGHT" if "mini" in model.lower() or "nano" in model.lower() or "8b" in model.lower() else "STANDARD"
...
async def _nvidia(settings: Settings) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        tasks = [_probe_nvidia_model(client, settings.nvidia_base_url, selected_headers, model) for model in candidate_models]
        results = await asyncio.gather(*tasks)
