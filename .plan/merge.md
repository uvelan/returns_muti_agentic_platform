...
| actorId required | feat/actorid-required | IN_PROGRESS | — | — |
...
- **`actorId` optionality — DISPATCHED** (`feat/actorid-required`). RV diagnosed it precisely: the generated schema types `actorId?: (string | null) | undefined` while the consumer requires `actorId: string | null`, because S1b's `actorId` reached `CaseFactProjection` **with a Python default**, so FastAPI marks it non-required and `openapi-typescript` renders it optional. **The fix is to make the field required in Python, NOT to patch the three fixtures** — S1b's rule is that `actorId` is always written, `None` included, so "no actor" is distinguishable from "field absent"; patching fixtures would leave the REST view free to omit the key, which is the gap that rule closes. **The deeper lesson, and the reason this survived a merge: it is a typecheck error that no suite gates on.** The dispatch asks whether `tsc` is run by any script; if not, wiring it is a decision for me.
