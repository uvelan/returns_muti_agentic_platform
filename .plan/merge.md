...
| V2 phase 2 (frontend) | feat/v2-frontend | CHANGES_REQUIRED (F1) → fixing · was b05a0bb1 | 1 · CR (335ce003) | — |
...
- **`tsc -b` has 3 pre-existing errors, all an `actorId` optionality mismatch** (verified pre-existing by stash: 3 before, 3 after the guard branch). Smells related to S1 phase 1b's `actorId` addition — the generated type is likely optional where a consumer expects required. RV asked to confirm and advise whether it needs a dispatch. **Note it does not fail any suite**, which is itself worth noticing: a typecheck error nothing gates on.
