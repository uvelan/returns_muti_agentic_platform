...
| S2 | feat/s2-delivery-spine | CHANGES_REQUIRED r2 (N1 only) → fixing · was c884e8e | 2 · CR (db7bfb9) → CR (faefa84) | — |
...
*Review round 2 — all three round-1 findings **withdrawn**, each re-injected by RV, including a "plausible wrong fix" probe (repair via `ensure_case_workflow`) that the suite correctly rejects — so `bind_case_workflow` is forced, not merely preferred. The rebuilt queue double was checked clause-by-clause against the shipped query and its one divergence is in the strict direction. **New: N1 (rule 10)** — the race test's blind counter is load-bearing upward but NOT downward: setting it to 0 leaves the test passing while it degrades into a duplicate of another test, silently unguarding F2. The number is coupled to production reads and already moved 2→1 this round, so the next such change unguards F2 invisibly while the comment claims otherwise. Advisory A1: deleting the `_CanonicalEditLockLost` raise leaves the suite green while the marker clears for an edit never written — half of F3 unpinned. Caveat: `_repair_workflow_link`'s `except Exception` would degrade silently back to round-1 starvation if a port omitted the method.*
...
5a. **Residual F3 exposure (from S2 review round 2).** RV upheld S2's choice to leave edit rows outside the marker transaction, having narrowed the real exposure to: process *death* in the insert→flag window, no later edit, then a direct `approve()`. Narrow, but it is V1's `approve()` that meets it — handle or explicitly accept it.
