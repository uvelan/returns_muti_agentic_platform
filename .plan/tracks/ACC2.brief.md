...
- **10 — DEFERRED per AMENDMENT-8, and the deferral is a test, not a silence.** The tool rung is deliberately unreachable in this deployment (see §1a). **Do not attempt to exercise it.** Instead **assert the unreachability**: read `compiled_rungs` against the released `support_resolver.tool_bindings` and prove the tool rung is absent from the compiled graph, from the target map, and from the released config — the three places that must agree. A deferred gate item becomes a verified fact. If a future release wires the ports, that test fails and item 10 comes back into scope, which is exactly what should happen.
- **9, 11–12 resolver:** facts-answerable → autonomous answer with provenance **and disclosure line**; tool route with no credential in checkpoints or logs; unresolvable → verbatim clarification that relays and unblocks; sub-threshold → no answer; gated reply visible and approvable; budget exhaustion; authz 403/404 with no fact written.
