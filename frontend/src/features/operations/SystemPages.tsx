  ...
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Governed operations"><p className="text-sm text-slate-600">Apply is idempotent. Reset removes generated operational state, then reapplies the canonical source dataset.</p><div className="mt-5 flex flex-wrap gap-3"><button className={primaryButton} disabled={mutation.isPending} type="button" onClick={() => { mutation.mutate(false); }}><DatabaseZap size={16} /> Apply seed</button><button className={dangerButton} disabled={mutation.isPending} type="button" onClick={() => { if (window.confirm("Reset all generated operational returns, events, support cases, and AI traces?")) mutation.mutate(true); }}><RefreshCw size={16} /> Reset generated state</button></div>{seed.validationErrors.length > 0 && <ul className="mt-5 list-disc space-y-1 pl-5 text-sm text-red-700">{seed.validationErrors.map((error) => <li key={error}>{error}</li>)}</ul>}</Panel>
      </div>
    </div>
  );
}
