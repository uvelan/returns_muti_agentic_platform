...
export function AgentDrawer() {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/55 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) ui.closeChat(); }}>
    <aside className="flex h-full w-full max-w-md flex-col border-l border-emerald-900 bg-[#0a1714] shadow-2xl" aria-label="Graph Schema Analyzer Agent">
      ...
      <form className="border-t border-emerald-950 p-4" onSubmit={(event) => { event.preventDefault(); submit(); }}><div className="flex items-end gap-2 rounded-xl border border-emerald-900 bg-[#07120f] p-2 focus-within:border-emerald-500"><textarea value={input} aria-label="Message to the Analyzer Agent" onChange={(event) => { setInput(event.target.value); }} rows={2} placeholder="Ask about the active selection…" className="min-h-12 flex-1 resize-none bg-transparent px-2 py-1 text-sm text-white outline-none placeholder:text-slate-400" /><button type="submit" disabled={ask.isPending || input.trim().length === 0} className="grid size-9 place-items-center rounded-lg bg-emerald-400 text-emerald-950 disabled:opacity-40" aria-label="Send message"><CornerDownLeft size={16} /></button></div></form>
    </aside>
  </div>;
}
