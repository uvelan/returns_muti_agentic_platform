...
import {
  ...
  generateAIStudioPromptProposal,
  getAIStudioProposal,
  getSchemaRegistry,
    ...
    <details className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <summary className="cursor-pointer list-none p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex gap-2"><Badge>{asset.fields.length} fields</Badge>{asset.writable_in_sandbox ? <Badge>Workspace writable</Badge> : null}</div>
        </div>
      </summary>
...
export function SchemaCatalogPage() {
  ...
  return <div><PageHeader title="Model & Schema Catalog" description="Version-controlled MongoDB collections, SQL tables, Neo4j nodes, relationships, ownership, keys, and isolated workspace write boundaries." />
...
export function AIStudioPage() {
  ...
  const [scenarioName, setScenarioName] = useState("return-reference-data");
  ...
  const promptGeneration = useMutation({
    mutationFn: generateAIStudioPromptProposal,
    onSuccess: (proposal) => {
      setPreviewId(proposal.id);
      void queryClient.invalidateQueries({ queryKey: studioKey });
    },
  });
  ...
  return <div><PageHeader title="AI Studio" description="Describe coherent synthetic data in plain language. The assistant creates a digest-bound review proposal and saves data only after explicit approval." />
    <section className="mb-6 rounded-xl border border-sky-200 bg-sky-50 p-5">
      <h2 className="flex items-center gap-2 font-semibold text-sky-950"><Bot size={18} />Ask the data agent</h2>
      <p className="mt-1 text-sm text-sky-800">Relationship-aware requests include linked customers, products, orders, shipments, warehouses, and bay configuration. Only a representative sample is generated for review; approval materializes the full source/reference dataset in the dedicated isolated workspace.</p>
      <p className="mt-2 text-xs text-sky-700">Returns, fulfillment, tracking, support tickets, bay assignments, and feedback remain workflow-owned and are created only when their operation or support agent runs.</p>
      <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto]"><label className="text-sm font-medium text-sky-950">Data request<textarea rows={3} className="mt-1 w-full rounded-md border border-sky-300 bg-white px-3 py-2" value={prompt} onChange={(event) => { setPrompt(event.target.value); }} /></label><button type="button" className={`${primaryButton} self-end`} disabled={promptGeneration.isPending || prompt.trim().length < 10} onClick={() => { promptGeneration.mutate({ prompt, seed, scenarioName }); }}><Bot size={16} />{promptGeneration.isPending ? "Planning..." : "Create review proposal"}</button></div>
      {promptGeneration.isError ? <p className="mt-3 text-sm text-red-700">{promptGeneration.error.message}</p> : null}
    </section>
    <div className="grid gap-6 xl:grid-cols-[1fr_1.2fr]"><section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="flex items-center gap-2 font-semibold"><Database size={18} />Advanced proposal builder</h2><div className="mt-4 grid gap-4"><label className="text-sm">Scenario name<input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" value={scenarioName} onChange={(event) => { setScenarioName(event.target.value); }} /></label><div className="grid grid-cols-2 gap-3"><label className="text-sm">Records per asset<input type="number" min={1} max={500} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" value={recordsPerAsset} onChange={(event) => { setRecordsPerAsset(Number(event.target.value)); }} /></label><label className="text-sm">Deterministic seed<input type="number" min={0} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" value={seed} onChange={(event) => { setSeed(Number(event.target.value)); }} /></label></div><fieldset><legend className="text-sm font-medium">Assets</legend><div className="mt-2 max-h-80 space-y-2 overflow-auto rounded-lg border border-slate-200 p-3">{allAssets.map((asset) => <label key={asset.asset_id} className="flex items-start gap-2 text-sm"><input type="checkbox" checked={selected.includes(asset.asset_id)} onChange={() => { toggle(asset.asset_id); }} /><span><span className="flex flex-wrap items-center gap-2"><strong>{asset.asset_id}</strong><Badge>{asset.writable_in_sandbox ? "Workspace apply allowed" : "Proposal only"}</Badge></span><span className="block text-xs text-slate-500">{asset.description}</span></span></label>)}</div></fieldset><button type="button" className={primaryButton} disabled={selected.length === 0 || generation.isPending} onClick={() => { generation.mutate({ assetIds: selected, recordsPerAsset, seed, mode: "DETERMINISTIC", scenarioName }); }}><Bot size={16} />Generate proposal</button>{generation.isError ? <p className="text-sm text-red-700">{generation.error.message}</p> : null}</div></section>
    <section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="font-semibold">Proposal history</h2>{(proposals.data?.length ?? 0) === 0 ? <div className="mt-4"><EmptyState title="No proposals" description="Ask the data agent or select model assets to generate a governed proposal." /></div> : <div className="mt-4 space-y-3">{proposals.data?.map((proposal) => <div key={proposal.id} className="rounded-lg border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><strong>{proposal.scenarioName}</strong><p className="mt-1 text-xs text-slate-500">{new Date(proposal.createdAt).toLocaleString()} · {proposal.assetIds.length} assets</p>{proposal.generationPrompt ? <p className="mt-2 text-sm text-slate-700">“{proposal.generationPrompt}”</p> : null}</div><Badge>{proposal.status}</Badge></div>{Object.keys(proposal.generationPlan).length > 0 ? <div className="mt-3 rounded-lg bg-sky-50 p-3 text-sm text-sky-900"><strong>Agent plan:</strong> {proposal.generationPlan.customers.toLocaleString()} customers × {proposal.generationPlan.ordersPerCustomer.toLocaleString()} orders = {proposal.generationPlan.totalOrders.toLocaleString()} linked orders. Only the preview sample exists until approval.</div> : null}<p className="mt-3 break-all font-mono text-[11px] text-slate-500">{proposal.digest}</p><div className="mt-3 flex flex-wrap gap-2">{Object.entries(proposal.recordCounts).map(([asset, count]) => <Badge key={asset}>{asset}: {count.toLocaleString()}</Badge>)}</div><div className="mt-4 flex flex-wrap gap-2"><button type="button" className={secondaryButton} onClick={() => { setPreviewId(proposal.id); }}>Review sample</button>{proposal.status !== "APPLIED" ? <button type="button" className={secondaryButton} disabled={apply.isPending} onClick={() => { if (window.confirm(`Approve this digest-bound proposal and save ${Object.values(proposal.recordCounts).reduce((sum, count) => sum + count, 0).toLocaleString()} records to the dedicated isolated workspace?`)) apply.mutate(proposal); }}><CheckCircle2 size={16} />Approve and save</button> : null}</div>{proposal.appliedAssets.length > 0 ? <p className="mt-3 text-xs text-emerald-700">Applied: {proposal.appliedAssets.join(", ")}</p> : null}{proposal.blockedAssets.length > 0 ? <p className="mt-3 text-xs text-amber-700">Proposal only by ownership rules: {proposal.blockedAssets.join(", ")}</p> : null}{Object.keys(proposal.applyErrors).length > 0 ? <p className="mt-3 text-xs text-red-700">Apply failures: {Object.entries(proposal.applyErrors).map(([asset, error]) => `${asset} (${error})`).join(", ")}</p> : null}</div>)}</div>}</section></div>
