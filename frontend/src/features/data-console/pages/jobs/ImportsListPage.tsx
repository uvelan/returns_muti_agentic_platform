...
export function ImportsListPage() {
  ...
  const { data, isLoading, isError, error } = useJobsList({ type: "IMPORT" });

  if (isLoading) return <LoadingState message="Loading imports..." />;
  if (isError) return <ErrorState title="Failed to load imports" message={error instanceof Error ? error.message : "Unknown error"} />;

  const jobs = data ?? [];

  return (
    <div className="p-6">
      <PageHeader title="Imports" description="History of all data imports.">
        <button
          onClick={() => { setLocation("/data-console/imports/new"); }}
      ...
      <DataTable<Job>
        ...
        columns={[
          ...
          { header: "Target", accessor: (row) => row.target },
